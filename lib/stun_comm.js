...
StunComm.prototype.sendStunRequest = function (bytes, onResponse, onFailure) {
  ...
  if (this._ongoingSendOperations[tid] === undefined) {
    this._ongoingSendOperations[tid] = {
  ...
  if (!this._transport.isReliable()) {
    ...
    if (this._ongoingSendOperations[tid].remainingRetransmissionAttempts === undefined) {
      this._ongoingSendOperations[tid].remainingRetransmissionAttempts = MAX_RETRANSMISSIONS
    }
    // start retransmission timer
    var timeout = setTimeout(
      function () {
        if (self._ongoingSendOperations[tid].remainingRetransmissionAttempts === 0) {
          // stopping retransmission
          var errorMsg = 'giving up, no more retransmission attempts left'
          self._log.error(errorMsg)
          self._error(errorMsg, onFailure)
          delete self._ongoingSendOperations[tid]
        } else {
          self._ongoingSendOperations[tid].remainingRetransmissionAttempts--
          self.sendStunRequest(bytes, onResponse, onFailure)
        }
      },
      self._transport.getTimeoutDelay()
    )
    self._ongoingSendOperations[tid].retransmissionTimeout = timeout
  }
}
...
StunComm.prototype.onIncomingStunResponse = function (stunPacket) {
  ...
  var timeout = this._ongoingSendOperations[stunPacket.tid].retransmissionTimeout
  if (timeout) {
    clearTimeout(timeout)
  }
  // this is a stun reply
  var onResponseCallback = this._ongoingSendOperations[stunPacket.tid].onResponse
  if (onResponseCallback) {
    onResponseCallback(stunPacket)
    delete this._ongoingSendOperations[stunPacket.tid]
  } else {
